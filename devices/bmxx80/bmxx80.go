...
func (d *Dev) Sense(e *rwio.Env) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.stop != nil {
...
func (d *Dev) SenseContinuous(interval time.Duration) (<-chan rwio.Env, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.stop != nil {
...
func (d *Dev) sensingContinuous(interval time.Duration, sensing chan<- rwio.Env, stop <-chan struct{}) {
