...
func (d *Dev) sense280(e *rwio.Env) error {
	...
	e.Temperature = rwio.Celsius(t * 10)

	if d.opts.Pressure != Off {
		p := d.cal280.compensatePressureInt64(pRaw, tFine)
		e.Pressure = rwio.KPascal((int32(p) + 127) / 256)
	}
	...
	if d.opts.Humidity != Off {
		...
		e.Humidity = rwio.RelativeHumidity((int32(h)*100 + 511) / 1024)
	}
	return nil
}
