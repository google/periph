...
// New returns a peripheral object that communicates over I²C to the
// DS2482/DS2483 controller.
//
// This peripheral object implements onewire.Bus and can be used to
// access peripherals on the bus.
func New(i i2c.Bus, opts *Opts) (*Dev, error) {
	...
	if opts != nil {
		...
		case 0x00:
		default:
			return nil, errors.New("ds248x: given address not supported by peripheral")
		}
	}
...
func (d *Dev) makeDev(opts *Opts) error {
	...
	// Write the peripheral configuration register to get the chip out of reset
	// state, immediately read it back to get confirmation.
	...
	if err := d.i2c.Tx([]byte{cmdWriteConfig, d.confReg}, dcr[:]); err != nil {
		return fmt.Errorf("ds248x: error while writing peripheral config register: %s", err)
	}
	// When reading back we only get the bottom nibble
	if dcr[0] != d.confReg&0x0f {
		return fmt.Errorf("ds248x: failure to write peripheral config register, wrote %#x got %#x back",
			d.confReg, dcr[0])
	}

	// Set the read ptr to the port configuration register to determine whether
	// we have a ds2483 vs ds2482-100. This will fail on peripherals that do not
	// have a port config register, such as the ds2482-100.
...
const (
	...
	cmdWriteConfig = 0xd2 // write the peripheral configuration
	...
	regDCR    = 0xc3 // read ptr for peripheral configuration register
