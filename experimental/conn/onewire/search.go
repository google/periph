...
type BusSearcher interface {
	...
	// waits for it to complete and returs the result.
	SearchTriplet(direction byte) (TripletResult, error)
}

type TripletResult interface {
...
func Search(bus BusSearcher, alarmOnly bool) ([]uint64, error) {
	...
	for {
		...
		for bit := 0; bit < 64; bit++ {
			...
			if !result.GotZero() && !result.GotOne() {
				return devices,
					fmt.Errorf("1-wire: devices disappeared during search")
			}
			...
			device = device | (uint64(result.Taken()) << uint(bit))
