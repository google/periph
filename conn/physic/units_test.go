...
func TestAngle_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"1Degrees", Degree},
		{"-1Degrees", -1 * Degree},
		{"180.00Degrees", 180 * Degree},
		{"0.5Degrees", 8726647 * NanoRadian},
		{"0.5°", 8726647 * NanoRadian},
		{"1nRadians", NanoRadian},
		{"1uRadians", MicroRadian},
		{"1mRadians", MilliRadian},
		{"0.5uRadians", 500 * NanoRadian},
		{"0.5mRadians", 500 * MicroRadian},
		{"200uRadians", 200 * MicroRadian},
		{"1Radians", Radian},
		{"1Pi", Pi},
		{"1nπ", 3 * NanoRadian},
		{"1pi", Pi},
		{"2Pi", 2 * Pi},
		{"-2Pi", -2 * Pi},
		{"0.5Pi", 1570796327 * NanoRadian},
		{"200mRadians", 200 * MilliRadian},
		{"20.0mRadians", 20 * MilliRadian},
		{"1nRadian", 1 * NanoRadian},
		{"1nradians", 1 * NanoRadian},
		{"1uradians", 1 * MicroRadian},
		{"1mradians", 1 * MilliRadian},
	}
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("Angle.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Angle.Set(%s) got expected error but got none", tt.in)
		}
	}
}
...
func TestDistance_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.want {
			t.Errorf("distance.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.want, tt.want, got, got)
		}
		if err != nil {
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("distance.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestElectricalCapacitance_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("ElectricalCapacitance.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("ElectricalCapacitance.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestElectricCurrent_Set(t *testing.T) {
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("ElectricCurrent.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestTemperature_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"1nK", 1 * NanoKelvin},
		{"10nK", 10 * NanoKelvin},
		{"100nK", 100 * NanoKelvin},
		{"1uK", 1 * MicroKelvin},
		{"10uK", 10 * MicroKelvin},
		{"100uK", 100 * MicroKelvin},
		{"1µK", 1 * MicroKelvin},
		{"10µK", 10 * MicroKelvin},
		{"100µK", 100 * MicroKelvin},
		{"1mK", 1 * MilliKelvin},
		{"10mK", 10 * MilliKelvin},
		{"100mK", 100 * MilliKelvin},
		{"1K", 1 * Kelvin},
		{"10K", 10 * Kelvin},
		{"100K", 100 * Kelvin},
		{"1kK", 1 * KiloKelvin},
		{"10kK", 10 * KiloKelvin},
		{"100kK", 100 * KiloKelvin},
		{"1MK", 1 * MegaKelvin},
		{"10MK", 10 * MegaKelvin},
		{"100MK", 100 * MegaKelvin},
		{"1GK", 1 * GigaKelvin},
		{"0C", ZeroCelsius},
		{"0°C", ZeroCelsius},
		{"20C", ZeroCelsius + 20*Kelvin},
		{"-20C", ZeroCelsius - 20*Kelvin},
		{"0F", 255372222222 * NanoKelvin},
		{"1F", ZeroFahrenheit + Fahrenheit}, // rounding
		{"0F", ZeroFahrenheit},
		{"33F", 273705555556 * NanoKelvin},
		{"32F", ZeroCelsius},
		{"-40F", ZeroCelsius - 40*Kelvin},
		{"212F", 373150000000 * NanoKelvin},
		{"-40F", 233150000000 * NanoKelvin},
		{"5000F", 3033150000000 * NanoKelvin},
		{"18446F", 10503150000000 * NanoKelvin},
		{"-459.67F", 0 * NanoKelvin},
	}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Temperature.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestElectricPotential_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("ElectricPotential.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("ElectricPotential.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestElectricResistance_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("ElectricResistance.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("ElectricResistance.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestPower_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"10W", 10 * Watt},
		{"10w", 10 * Watt},
		{"12.23w", 12230 * MilliWatt},
		{"12.23uw", 12230 * NanoWatt},
	}
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("Power.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Power.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestEnergy_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("Energy.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Energy.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestPressure_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("Pressure.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Pressure.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestLuminousIntensity_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("LuminousIntensity.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("LuminousIntensity.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestLuminousFlux_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("LuminousFlux.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("LuminousFlux.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestSpeed_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"1nm/s", 1 * NanoMetrePerSecond},
		{"10nm/s", 10 * NanoMetrePerSecond},
		{"100nm/s", 100 * NanoMetrePerSecond},
		{"1um/s", 1 * MicroMetrePerSecond},
		{"10um/s", 10 * MicroMetrePerSecond},
		{"100um/s", 100 * MicroMetrePerSecond},
		{"1µm/s", 1 * MicroMetrePerSecond},
		{"10µm/s", 10 * MicroMetrePerSecond},
		{"100µm/s", 100 * MicroMetrePerSecond},
		{"1mm/s", 1 * MilliMetrePerSecond},
		{"10mm/s", 10 * MilliMetrePerSecond},
		{"100mm/s", 100 * MilliMetrePerSecond},
		{"1m/s", 1 * MetrePerSecond},
		{"10m/s", 10 * MetrePerSecond},
		{"100m/s", 100 * MetrePerSecond},
		{"1km/s", 1 * KiloMetrePerSecond},
		{"10km/s", 10 * KiloMetrePerSecond},
		{"100km/s", 100 * KiloMetrePerSecond},
		{"1Mm/s", 1 * MegaMetrePerSecond},
		{"10Mm/s", 10 * MegaMetrePerSecond},
		{"100Mm/s", 100 * MegaMetrePerSecond},
		{"1Gm/s", 1 * GigaMetrePerSecond},
		{"1km/h", 1 * KilometrePerHour},
		{"10km/h", 2777777778 * NanoMetrePerSecond},
		{"100km/h", 27777777778 * NanoMetrePerSecond},
		{"1000km/h", 277777777778 * NanoMetrePerSecond},
		{"-1km/h", -1 * KilometrePerHour},
		{"-10km/h", -2777777778 * NanoMetrePerSecond},
		{"-100km/h", -27777777778 * NanoMetrePerSecond},
		{"-1000km/h", -277777777778 * NanoMetrePerSecond},
		{"1mph", 1 * MilePerHour},
		{"1fps", 1 * FootPerSecond},
		{"1000nfps", 305 * NanoMetrePerSecond},
		{"-1000nfps", -305 * NanoMetrePerSecond},
		{"100nmph", 45 * NanoMetrePerSecond},
		{"-100nmph", -45 * NanoMetrePerSecond},
		{"100km/h", 27777777778 * NanoMetrePerSecond},
	}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Speed.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestMass_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"1uoz", 28350 * NanoGram},
		{"-1uoz", -28350 * NanoGram},
		{"1moz", 28349523 * NanoGram},
		{"10moz", 283495231 * NanoGram},
		{"100moz", 2834952313 * NanoGram},
		{"1000moz", 1 * OunceMass},
		{"1Oz", 1 * OunceMass},
		{"1MOz", 1000000 * OunceMass},
		{"-1MOz", -1000000 * OunceMass},
		{"1lb", 1 * PoundMass},
		{"-1lb", -1 * PoundMass},
		{"1tonne", 1 * Tonne},
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Mass.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestForce_Set(t *testing.T) {
	...
	for _, tt := range succeeds {
		...
		if got != tt.expected {
			t.Errorf("Force.Set(%s) wanted: %v(%d) but got: %v(%d)", tt.in, tt.expected, tt.expected, got, got)
		}
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("Force.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestRelativeHumidity_Set(t *testing.T) {
	succeeds := []struct {
		...
		{"1%rh", 1 * PercentRH},
		{"55.0%rh", 55 * PercentRH},
		{"1%rh", 1 * PercentRH},
		{"1%rh", 1 * PercentRH},
		{"0.00001%rH", 1 * TenthMicroRH},
		{"0.001%rH", 1 * PercentRH / 1000},
		{"1mrH", 1 * MilliRH},
		{"10urH", 10 * MicroRH},
		{"1µrH", 1 * MicroRH},
		{"-1µrH", -1 * MicroRH},
		{"1urH", 1 * MicroRH},
		{"0.1%rH", (1 * PercentRH) / 10},
		{"1%rH", 1 * PercentRH},
		{"-1%rH", -1 * PercentRH},
	}
	...
	for _, tt := range fails {
		...
		if err == nil {
			t.Errorf("RelativeHumidity.Set(%s): expected err but got none", tt.in)
		}
	}
}
...
func TestMeta_Set(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			"parse error: no units provided, need: \"m, Metre, Mile, Inch, Foot or Yard\"",
		},
		{
