...
// bus for peripherals. Buses that implement this interface can be searched with
// the Search function.
...
type TripletResult struct {
	GotZero bool  // a peripheral with a zero in the current bit position responded
	GotOne  bool  // a peripheral with a one in the current bit position responded
...
func Search(bus BusSearcher, alarmOnly bool) ([]Address, error) {
	var devices []Address // peripherals we're finding
	lastDiscrepancy := -1 // how far we need to repeat the same ID in the next iteration
	var lastDevice uint64 // ID of last peripheral found

	// Loop to do the search. Each iteration detects one peripheral.
	for {
		...
		if err := bus.Tx([]byte{cmd}, nil, WeakPullup); err != nil {
			// Expect an NoDevicesError if no peripheral is present on the bus and
			// pass that back.
			return devices, err
		}
		...
		var device uint64   // ID of current peripheral
		var idBytes [8]byte // ID of peripheral as bytes
		for bit := 0; bit < 64; bit++ {
			// Decide which direction to search into: 0 or 1.
			var dir byte
			if bit < lastDiscrepancy {
				// We haven't reached the last discrepancy yet, so we
				// need to repeat the bits of the last peripheral.
			...
			// Check for the absence of peripherals on the bus. This is a 1-wire
			// bus error condition and we return a partial result.
			if !result.GotZero && !result.GotOne {
				return devices, fmt.Errorf("onewire: peripherals disappeared during search")
			}

			// Check whether we have peripherals responding for 0 and 1 and we
			// picked 0.
			if result.GotZero && result.GotOne && result.Taken == 0 {
				discrepancy = bit
			}

			// Shift a bit into the current peripheral's ID
		...
		// Verify the CRC and record peripheral if we got it right.
		if !CheckCRC(idBytes[:]) {
		...
		if lastDiscrepancy = discrepancy; lastDiscrepancy == -1 {
			return devices, nil // we reached the last peripheral
		}
	}
}
